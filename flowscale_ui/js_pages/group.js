...
/*	alert(document.getElementById('add_group_name').value +" input switch "+ 
	...
	*/
	...
	var transport_type = "";
	var transport_direction = "";
	for (i =0; i< document.transport_data.transport_type.length; i++){
		if(document.transport_data.transport_type[i].checked){
			transport_type = document.transport_data.transport_type[i].value;
			break;
		}
	}
	for (i =0; i< document.transport_data.transport_direction.length; i++){
		if(document.transport_data.transport_direction[i].checked){
			transport_direction = document.transport_data.transport_direction[i].value;
			break;
		}
	}
	var ds =  new YAHOO.util.DataSource("webservice/admin.cgi?action=add_group&group_name="+ group_name +"&input_switch="+input_switch+
								...
								+"&maximum_allowed="+maximum_allowed+"&transport_type="+transport_type+"&transport_direction="+transport_direction);
 ...
 //     {key: "port_address", label: "Port Address", width: 100},
     ...
     var configs = {
		...
		if(id_value == 1){
			...
			output_table_add.subscribe('initEvent',function() {
...
/***************************************************group output port table *******************************************/		
  ...
  //      {key: "port_address"},
	...
	//	width : '310px'
			...
			addedTablePorts[arrayIndex].subscribe('initEvent',function() {
...
//		width : '310px'
			...
			output_ports_table_[% group.group_id %].subscribe('initEvent',function() {
