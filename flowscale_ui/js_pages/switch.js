...
var configs = {
		height : '10em',
			...
			var port_table = new
			YAHOO.widget.ScrollingDataTable('port_table_'+switch_id, columns,ds,{height:"10em",selectionMode:"singlecell"});
			port_table.subscribe("cellMouseoverEvent", port_table.onEventHighlightCell); 
			port_table.subscribe("cellMouseoutEvent", port_table.onEventUnhighlightCell); 
port_table.subscribe("cellClickEvent", port_table.onEventSelectCell); 
	   port_table.subscribe("cellSelectEvent", port_table.clearTextSelection); 
...
/*	YAHOO.util.Event.onContentReady("offload_port_[% switch.datapath_id %]", function() {
		var offload_button = new YAHOO.widget.Button("offload_port_[% switch.datapath_id %]", {
			label : "Offload Port"
		});

		offload_button.addClass("button_panel");

//		YAHOO.util.Event.addListener("offload_port_[% switch.datapath_id %]", "click", offload_port, null , true);
	});
...
function offload_port(datapath_id){

var port_to_offload = document.getElementById('portValue_'+datapath_id).value;
var interval = document.getElementById('interval_'+datapath_id).value;

var ds =  new YAHOO.util.DataSource("webservice/admin.cgi?action=offload_port&port="+port_to_offload+"&interval="+interval);
