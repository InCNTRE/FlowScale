...
sub add_new_group {
	...
	my $transport_type = $args{'transport_type'};
	my $transport_direction = $args{'transport_direction'};
	my @input_ports_array  = split( ',', $input_ports );
	my @output_ports_array = split( ',', $output_ports );
	my @group_values_array = split( ',', $group_values );

	my $query =
"insert into flow_group (input_switch, output_switch, comments, priority, type, maximum_flows,network_protocol,transport_direction) values (?,?,?,?,?,?,?,?)";

	my $sth = $self->_prepare_query($query) or return undef;
	$sth->execute( $input_switch, $output_switch, $group_name, $priority,
		$group_type, $maximum_allowed, $transport_type, $transport_direction);
...
sub get_group_output_ports {
	...
	my $sth = $self->_prepare_query("select group_port.port_id , flow_group.output_switch from group_port, flow_group  where flow_group.group_id = ?   and port_direction = 1 and group_port.group_id = ?");
...
#		  . " and group_port.port_id = switch_port.port_id and port_direction = 1 and group_port.group_id = ?"
	...
	while ( my $group_row = $sth->fetchrow_hashref() ) {
		...
		push(
			...
			{
				"port_id",      $group_row->{'port_id'},
				"port_address", $group_row->{'output_switch'}
			}
		);
