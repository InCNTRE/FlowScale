...
sub add_new_group {
	...
	$client->addHeader('networkProtocol',      $args{'transport_type'});
	$client->addHeader('transportDirection', $args{'transport_direction'});
	$client->GET('/');
...
sub offload_port{
	...
	$client->addHeader('action', 'offloadPort');
	$client->addHeader('port', $port);
	$client->addHeader('interval', $interval);
