...
sub submit_new_group() {
	...
	my $transport_type = $cgi->param('transport_type');
	my $transport_direction = $cgi->param('transport_direction');
	...
	my $result = $db->add_new_group(
		...
		transport_direction => $cgi->param('transport_direction'),
	);
	if ( !defined $result ) {
		$results->{'error'} = $db->get_error();
...
sub offload_port(){
	...
	$results->{'offload_result'} = $ctrl->offload_port(port => $port,
							   interval => $interval,	);
